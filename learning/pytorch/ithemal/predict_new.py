...
def predict_compare(args, model, data, verbose):
    ...
    for row in rows:
        ...
        try:
            ...
            sql = 'SELECT time_actual from ' + args.ttable + ' WHERE code_id = ' + str(row[0])
...
def main():
    ...
    # arguments for time predict and compare from databse
    parser.add_argument('--database', help='database name to read xml code', action='store',type=str,default='')
    parser.add_argument('--user', help='database user name', action='store',type=str,default='')
    parser.add_argument('--password', help='database password', action='store',type=str,default='')
    parser.add_argument('--port', help='database port number', action='store',type=str,default='')
    parser.add_argument('--ctable', help='database code table name', action='store',type=str,default='')
    parser.add_argument('--ttable', help='database time table name', action='store',type=str,default='')
    ...
    input_group.add_argument('--predict_compare', help='Predict sub-blocks in sql database and compare with reference', action='store_true', default=False)
    ...
    if args.raw_stdin:
        ...
        if args.predict_compare: #read from database
            predict_compare(args, model, data, args.verbose)
        else:# read input binary file
